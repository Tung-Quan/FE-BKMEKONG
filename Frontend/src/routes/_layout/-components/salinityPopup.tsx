...
const WarningIcon = () => (
  <svg className="h-16 w-16" fill="currentColor" viewBox="0 0 20 20">
    <path
      fillRule="evenodd"
      d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 3.01-1.742 3.01H4.42c-1.53 0-2.493-1.676-1.743-3.01l5.58-9.92zM10 13a1 1 0 100-2 1 1 0 000 2zm-1-3a1 1 0 011-1h.01a1 1 0 110 2H10a1 1 0 01-1-1z"
      clipRule="evenodd"
    />
  </svg>
);
...
const getSalinityState = (
  salinityValue: string | number | null | undefined
) => {
  const s = Number(salinityValue);
...
export default function SalinityPopup({
  ...
  return (
    <div
      className={`w-[260px] overflow-hidden rounded-lg shadow-lg ${state.colors.bg}`}
    >
      {/* Header */}
      <div className={`p-2 ${state.colors.header}`}>
        <h3 className="text-center text-base font-bold uppercase text-white">
      ...
      <div className="relative p-3">
        ...
        <div
          className={`absolute bottom-0 right-1 opacity-20 ${state.colors.accent}`}
        >
          {state.icon}
        </div>
        ...
        <p className="text-sm text-gray-800">
          Độ mặn&nbsp;&nbsp;&nbsp;:{' '}
          <span className="font-bold">{formattedSalinity} g/L</span>
        </p>
      ...
      {secondPopup && (
        <button type="button" className="w-full" onClick={secondPopup}>
          <div
            className="w-full cursor-pointer bg-yellow-100 py-2 text-center text-yellow-800 hover:bg-yellow-200"
            onClick={secondPopup}
          >
